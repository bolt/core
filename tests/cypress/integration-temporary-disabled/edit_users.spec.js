...
describe('Edit user successfully, Edit users incorrectly', () => {
    it('checks that an admin can edit users', () => {
        ...
        cy.get(
            'table:nth-child(1) > tbody > tr:nth-child(6) > td:nth-child(7)',
        ).click({ force: true });
        cy.wait(100);
        cy.get(
            'table:nth-child(1) > tbody > tr:nth-child(6) > td:nth-child(7) > div > div > a:nth-child(1)',
        )
        ...
        cy.get('input[name="user[email]"]').type(
            'tom_admin_changed@example.org',
        );
        ...
        cy.get('table:nth-child(1) > tbody > tr:nth-child(6)')
            .children('td')
            .eq(1)
            .should('contain', 'Tom Doe CHANGED');
    });
    ...
    it("checks that an admin can't edit a user with incorrect details", () => {
        ...
        cy.get('#field--user_plainPassword')
            .children('div')
            .eq(1)
            .should('contain', 'Suggested secure password');
    });
    ...
    it("checks that a user can't edit their profile with an incorrect display name", () => {
        ...
        cy.get('.profile__dropdown.dropdown-menu.dropdown-menu-right')
            .children('ul')
            .children('li')
            .eq(0)
            .should('exist');
