...
describe('As an Admin I want to view saved changes of a record or preview these', () => {
    it('checks if an admin can view saved changes on a record', () => {
        ...
        cy.get('input[id="field-heading"]').type(
            'This is the title in the wrong locale',
        );
        ...
        cy.get('input[id="field-heading"]').clear();
        cy.get('input[id="field-heading"]').type(
            'This is the title in the right locale',
        );
        cy.get('button[class="btn btn-success mb-0"]')
            .eq(1)
            .scrollIntoView();
    ...
    it('checks if an admin can preview an edited record', () => {
        ...
        cy.get('#button-preview')
            .invoke('removeAttr', 'formtarget')
            .click({ force: true });
