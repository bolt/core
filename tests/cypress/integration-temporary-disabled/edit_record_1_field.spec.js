...
describe('As an Admin I want to be able to make use of the embed, infobox and image fields', () => {
    it('checks if an admin can use the embed field', () => {
        ...
        cy.get('input[name="fields[embed][url]"').type(
            'https://www.youtube.com/watch?v=x4IDM3ltTYo',
        );
        cy.wait(2000);
        cy.get('button[class="btn btn-tertiary refresh"]').should('be.enabled');

        cy.get('input[name="fields[embed][title]"]').should(
            'have.value',
            'Silversun Pickups - Nightlight (Official Video)',
        );
        cy.get('input[name="fields[embed][authorname]"]').should(
            'have.value',
            'Silversun Pickups',
        );
        cy.get('input[name="fields[embed][width]"]').should(
            'have.value',
            '200',
        );
        cy.get('input[name="fields[embed][height]"]').should(
            'have.value',
            '113',
        );
    ...
    it('checks if an admin can see the infobox field', () => {
        ...
        cy.get('.popover-body')
            .should(
                'contain',
                'This is an info box shown as a popover next to the field label.',
            )
            .should('be.visible');
    });
