...
describe('As an Admin I want to be able to make use of the date & datetime fields', () => {
    before(() => {
        ...
        cy.get(
            '#multiselect-user_locale > div > div.multiselect__select',
        ).scrollIntoView();
        cy.get(
            '#multiselect-user_locale > div > div.multiselect__select',
        ).click();
    ...
    it('checks if an admin can use the date field', () => {
        ...
        cy.get('#field--field-date input.editor--date').should(
            'have.class',
            'active',
        );
        ...
        cy.get(
            '.flatpickr-calendar.open span[aria-label="March 15, 2019"]',
        ).click();

        cy.get('input[name="fields[date]"]').should(
            'have.value',
            '2019-03-15 00:00:00',
        );
    });
    ...
    it('checks if an admin can use the datetime field with an AM time (with AM/PM selector)', () => {
        ...
        cy.get(
            '#field--field-datetime button[aria-label="Date picker"]',
        ).click();
        cy.get('#field--field-datetime input.editor--date').should(
            'have.class',
            'active',
        );
        ...
        cy.get(
            '.flatpickr-calendar.open span[aria-label="May 10, 2020"]',
        ).click();

        cy.get('input[name="fields[datetime]"]').should(
            'have.value',
            '2020-05-10 11:30:00',
        );
    });
    ...
    it('checks if an admin can use the datetime field with a PM time (with AM/PM selector)', () => {
        ...
        cy.get(
            '.flatpickr-calendar.open span[aria-label="July 20, 2020"]',
        ).click();
    ...
    it('checks if an admin can use the datetime field with an AM time (without AM/PM selector)', () => {
        ...
        cy.get(
            '.flatpickr-calendar.open span[aria-label="janvier 28, 2020"]',
        ).click();
    ...
    it('checks if an admin can use the datetime field with a PM time (without AM/PM selector)', () => {
        ...
        cy.get('.flatpickr-calendar.open input.flatpickr-minute').type('30');
        cy.get(
            '.flatpickr-calendar.open span[aria-label="juillet 20, 2020"]',
        ).click();
