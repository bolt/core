...
describe('As an Admin I want to fill in a Set and an Collection', () => {
    it('checks if an admin can fill in a set', () => {
        ...
        cy.get('input[name="sets[set][title]"]')
            .its('length')
            .should('eq', 1);
        cy.get("#sets label[for='field-set-textarea']").should(
            'contain',
            'Textarea',
        );
        cy.get('textarea[name="sets[set][textarea]"]')
            .its('length')
            .should('eq', 1);
        ...
        cy.get('textarea[name="sets[set][textarea]"]').should(
            'have.value',
            'Bar',
        );
    });
    ...
    it('checks if an admin can fill in a collection', () => {
        ...
        cy.url().should('contain', '/bolt/edit/43');
        cy.get("label[for='field-collection']").should(
            'contain',
            'Collection:',
        );
        ...
        cy.get(
            '#field--field-collection > div > div > div > div.dropdown > div > a:nth-child(2)',
        ).click();

        cy.get('.action-move-up-collection-item')
            .eq(0)
            .should('be.disabled');
        ...
        cy.get('button[id="collection-dropdownMenuButton"]').click();
        cy.get(
            '#field--field-collection > div > div > div > div.dropdown > div > a:nth-child(1)',
        ).click();

        cy.get('.collection-item')
            .its('length')
            .should('eq', 4);
        cy.get('#collections textarea')
            .eq(1)
            .clear();
        cy.get('#collections textarea')
            .eq(1)
            .type('Bye, Bolt');
        cy.get(".collection-item input[type='text']")
            .eq(1)
            .clear();
        cy.get(".collection-item input[type='text']")
            .eq(1)
        ...
        cy.get(
            '.collection-item:nth-child(4) .action-move-down-collection-item',
        ).scrollIntoView();
        cy.get(
            '.collection-item:nth-child(4) .action-move-down-collection-item',
        ).click();
        ...
        cy.get('.collection-item:nth-child(4) .collection-item-title').should(
            'contain',
            'Set',
        );
        cy.get('.collection-item:nth-child(5) .collection-item-title').should(
            'contain',
            'Textarea',
        );

        cy.get(
            '.collection-item:nth-child(5) .action-remove-collection-item',
        ).scrollIntoView();
        cy.get(
            '.collection-item:nth-child(5) .action-remove-collection-item',
        ).click({ force: true });
        ...
        cy.get(
            '.collection-item:nth-child(4) .action-remove-collection-item',
        ).scrollIntoView();
        cy.get(
            '.collection-item:nth-child(4) .action-remove-collection-item',
        ).click({ force: true });
