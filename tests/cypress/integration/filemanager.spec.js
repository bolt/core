...
describe('As an Admin I am able to use the files section', () => {
    ...
    it('checks if an admin can delete files in the Files section', () => {
        cy.login();
        cy.visit('/bolt/filemanager/files');
        cy.get('#files-list tr td b')
            .eq(1)
            .get('a')
            .contains('_b-penguin.jpeg');

        cy.get(
            'button[class="btn btn-sm btn-secondary edit-actions__dropdown-toggler dropdown-toggle dropdown-toggle-split"]',
        )
            .eq(1)
            .click();
    ...
    it('checks if an admin can create and delete folders in the Files section', () => {
        ...
        cy.get('a[class="btn btn-danger btn-sm btn-mb-0 text-nowrap "]')
            .eq(0)
            .click();
        cy.wait(1000);
        cy.get('.modal-dialog').should('have.length', 1);
