...
describe('As an admin I should be able to run bulk actions', () => {
    it('checks if an admin can see all items', () => {
        ...
        cy.get(".listing__filter input[type='checkbox']").click({
            force: true,
        });

        cy.get('.is-primary').should('contain', '8');
        //cy.get('div[class="card-header"]').should('contain', 'Pages Selected');
        cy.get('.admin__body--aside .card-body .multiselect').should(
            'contain',
            'Select option',
        );
    ...
    it('checks if an admin can make multiple changes at once', () => {
        ...
        cy.get('.multiselect--active').should(
            'contain',
        ...
        cy.get(
            'aside .card-body .multiselect__content-wrapper > ul > li:nth-child(2)',
        ).click();
        cy.wait(100);
        ...
        cy.get('.listing__records .is-meta .status.is-draft')
            .its('length')
            .should('eq', 8);
        cy.get('.listing__records .is-meta .status.is-published').should(
            'not.exist',
        );
        cy.get('.listing__records .is-meta .status.is-held').should(
            'not.exist',
        );
        ...
        cy.get(
            'aside .card-body .multiselect__content-wrapper > ul > li:nth-child(1)',
        ).click();
        ...
        cy.get('.listing__records .is-meta .status.is-published')
            .its('length')
            .should('eq', 8);
        cy.get('.listing__records .is-meta .status-is-draft').should(
            'not.exist',
        );
        cy.get('.listing__records .is-meta .status.is-held').should(
            'not.exist',
