...
describe('As an Admin I want to see separators, placeholders and default values', () => {
    ...
    it('checks if an admin can see placeholder on new content', () => {
        ...
        cy.get('input[name="fields[title]"]')
            .should('have.attr', 'placeholder')
            .and('match', /Placeholder for the title/);
    });
    ...
    it('checks if an admin can see default values', () => {
        ...
        cy.get('input[name="fields[title]"]').should(
            'have.value',
            'Title of a test contenttype',
        );
        cy.get('input[name="fields[image][filename]"]').should(
            'have.value',
            'foal.jpg',
        );

        cy.get('input[name="sets[set_field][title]"]').should(
            'have.value',
            'This is the default title value',
        );
        cy.get('input[name="sets[set_field][year]"]').should(
            'have.value',
            '2020',
        );

        cy.get(
            'input[name="collections[collection_field][photo][1][filename]"]',
        ).should('have.value', 'kitten.jpg');
        cy.get(
            'input[name="collections[collection_field][photo][1][alt]"]',
        ).should('have.value', 'Cute kitten');

        cy.get(
            'input[name="collections[collection_field][paragraph][2]"]',
        ).should('have.value', 'An image, followed by some content');

        cy.get(
            'input[name="collections[collection_field][photo][3][filename]"]',
        ).should('have.value', 'joey.jpg');
        cy.get(
...
describe('As an Admin, I want to duplicate a page', () => {
    it('checks if an admin can duplicate a page', () => {
        ...
        cy.get('input[name="fields[slug]"]').should(
            'have.value',
            'this-is-a-page',
        );
